...
const getExercisesForSession = (type, exerciseDB={}, workoutTemplates={}) => {
  ...
  if(isMuscleBased) {
    ...
    candidates = Object.entries(exerciseDB)
      .filter(([,ex]) => ex.trainings.includes(type))
      .map(([name, ex]) => ({ name, muscle: ex.muscle, id: name }));
  }
        ...
        {showAdd&&(
          <div className="card" style={{marginBottom:12}}>
            ...
            <button className="btn btn-red" onClick={()=>{
              ...
              const newName = newEx.name.trim();
              setExerciseDB(prev=>({...prev,[newName]:{muscle:newEx.muscle,trainings:[],history:[]}}));
            ...
            <div key={muscle} className="card" style={{marginBottom:8}}>
              ...
              {exs.map(ex=>{
                ...
                const last = exData && exData.history.length > 0
                  ...
                  : null;
                const isCustom = !Object.values(EXERCISES).flat().some(e=>e.name===ex.name);
                return (
    ...
    if(entries.length) {
      setExerciseDB(prev=>{
        ...
        entries.forEach(e=>{
          ...
          } else if(!next[e.exercise].trainings.includes(e.type)) {
