...
const autoBackup = (history, dayLogs) => {
  try {
    const backup = { history, dayLogs, savedAt: new Date().toISOString() };
    localStorage.setItem("gymtracker_backup", JSON.stringify(backup));
  } catch {}
};
...
// ── EXERCISE ALIASES (old name -> canonical name + muscle) ───────────────────
// Needed because history entries from before rename still use old names
const EXERCISE_ALIASES = {
  "Rozpiętki":              {name:"Atlas rozpiętki",       muscle:"klata"},
  "Płaska":                 {name:"Ławka płasko",          muscle:"klata"},
  "Skos":                   {name:"Ławka skośnie hantle",  muscle:"klata"},
  "Barki bok":              {name:"Wznosy bokiem",         muscle:"barki"},
  "Barki góra":             {name:"Wznosy nad głowę",      muscle:"barki"},
  "Bark tył":               {name:"Rozpiętki tył",         muscle:"barki"},
  "Ściąganie góra szeroko": {name:"Wyciąg góra szeroko",   muscle:"plecy"},
  "Ściąganie góra wąsko":   {name:"Wyciąg góra wąsko",     muscle:"plecy"},
  "Ściąganie dół wąsko":    {name:"Wyciąg nisko wąsko",    muscle:"plecy"},
  "Biceps skos":            {name:"Biceps skos siedząco",  muscle:"biceps"},
  "Biceps modlitewnik":     {name:"Modlitewnik",           muscle:"biceps"},
  "Triceps góra":           {name:"Ławeczka triceps",      muscle:"triceps"},
  "Triceps dół":            {name:"Wyciąg triceps",        muscle:"triceps"},
  "Ławka płasko hantle":    {name:"Ławka płasko",          muscle:"klata"},
};

// Resolve exercise name and muscle (handles old names from history)
...
const MUSCLES_ORDER = ["klata","plecy","barki","biceps","triceps","nogi"];
...
export default function App() {
  ...
  const DEFAULT_WEEKLY_GOALS = {klata:9,barki:9,triceps:6,plecy:12,biceps:9,nogi:6};
    ...
    <>
      ...
      <div className={`app ${!settings.darkMode ? "light-mode" : ""}`}>
        <div className="screen">
          ...
          {tab==="calendar" && <ScreenCalendar calDate={calDate} setCalDate={setCalDate} workoutDates={workoutDates} dayLogs={dayLogs} history={history} selDay={selDay} setSelDay={setSelDay} customExercises={customExercises} weeklyGoals={weeklyGoals} setWeeklyGoals={setWeeklyGoals}/>}
        ...
        {showSettings && <SettingsModal settings={settings} toggle={toggleSetting} onClose={()=>setShowSettings(false)} history={history} dayLogs={dayLogs} weeklyGoals={weeklyGoals} setWeeklyGoals={setWeeklyGoals}/>}
      </div>
    </>
  );
}
...
// ── SETTINGS MODAL ────────────────────────────────────────────────────────────
function SettingsModal({settings,toggle,onClose,history,dayLogs,weeklyGoals={},setWeeklyGoals=()=>{}}) {
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginTop:16,marginBottom:8}}>
          <div style={{fontSize:13,fontWeight:700,color:"var(--muted2)",marginBottom:12,letterSpacing:.5}}>CELE SERII NA TYDZIEŃ</div>
          {Object.entries({klata:"Klatka",plecy:"Plecy",barki:"Barki",biceps:"Biceps",triceps:"Triceps",nogi:"Nogi"}).map(([m,label])=>{
            const MC = {klata:"#ef4444",plecy:"#3b82f6",barki:"#f97316",biceps:"#8b5cf6",triceps:"#eab308",nogi:"#22c55e"};
            const val = (weeklyGoals&&weeklyGoals[m]) ? weeklyGoals[m] : 9;
            return (
              <div key={m} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:"1px solid var(--border)"}}>
                <span style={{fontSize:14,color:MC[m],fontWeight:600}}>{label}</span>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <button onClick={()=>{ const nv=Math.max(1,val-1); setWeeklyGoals(prev=>({...prev,[m]:nv})); storage.set("weeklyGoals",{...weeklyGoals,[m]:nv}); }}
                    style={{width:40,height:40,borderRadius:10,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                  <span style={{fontFamily:"'Bebas Neue'",fontSize:26,minWidth:32,textAlign:"center",color:MC[m]}}>{val}</span>
                  <button onClick={()=>{ const nv=val+1; setWeeklyGoals(prev=>({...prev,[m]:nv})); storage.set("weeklyGoals",{...weeklyGoals,[m]:nv}); }}
                    style={{width:40,height:40,borderRadius:10,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                </div>
              </div>
            );
...
function ScreenTraining({history,setHistory,saveDay,todayStr,dayLogs,settings,customExercises,setCustomExercises}) {
  ...
  const deleteExercise = (ex) => {
    setCustomExercises(prev=>prev.filter(e=>
      e.name!==ex.name &&
      e.name!==ex.originalName &&
      e.editOf!==ex.name &&
      e.editOf!==ex.originalName
    ));
    setEditEx(null);
  };
...
function SessionView({type,history,setHistory,todayStr,onBack,saveDay,settings,customExercises=[]}) {
  ...
  const [activeRestEx,setActiveRestEx] = useState(null);
  const [restTimer,setRestTimer]     = useState(null);
  const wakeLockRef = useState(null);
  ...
  useEffect(()=>{
    ...
    if(restTimer<=0){ setRestTimer(null); setActiveRestEx(null); if(settings?.vibration) vibrate([200,100,200,100,200]); return; }
  ...
  const toggleSet=(exId,setIdx)=>{
    setDone(prev=>{
      ...
      if(next[setIdx]){
        ...
        setActiveRestEx(exId);
        if(settings?.vibration) vibrate([100]);
      } else if(activeRestEx===exId){
        setRestTimer(null); setActiveRestEx(null);
      }
  ...
  const handleFinish=()=>{
    const newEntries=exList.filter(ex=>weights[ex.id]).map(ex=>({
      date:todayStr, type, exercise:ex.name,
      weight:parseFloat(weights[ex.id]),
      sets:(done[ex.id]||[]).filter(Boolean).length || 3,
      id:`${todayStr}-${ex.id}-${Date.now()}`
    }));
  ...
  return (
    <div className="animate-up">
      ...
      {[...exList].sort((a,b)=>{
        const aDone=(done[a.id]||[]).filter(Boolean).length===SETS;
        const bDone=(done[b.id]||[]).filter(Boolean).length===SETS;
        ...
        const isResting=activeRestEx===ex.id&&restTimer!==null;
        return (
          <div key={ex.id} className="card" style={{opacity:exDone?0.7:1,transition:"opacity .3s",border:isResting?`1.5px solid ${color}`:undefined}}>
            ...
            {isResting&&(
              <div style={{background:color+"22",border:`1px solid ${color}44`,borderRadius:10,padding:"10px 14px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <div style={{fontSize:10,color,fontWeight:700,letterSpacing:1,marginBottom:2}}>⏸ PRZERWA</div>
                  <div style={{fontFamily:"'Bebas Neue'",fontSize:36,color,lineHeight:1}}>{fmtTime(restTimer)}</div>
                </div>
                <button onClick={()=>{setRestTimer(null);setActiveRestEx(null);}}
                  style={{background:color,border:"none",borderRadius:8,padding:"8px 14px",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:13}}>
                  Pomiń
                </button>
              </div>
            )}
...
function WeeklyMuscleProgress({history, customExercises=[], weeklyGoals, setWeeklyGoals}) {
  ...
  // Read directly from localStorage + props - always fresh
  const storedGoals = storage.get("weeklyGoals", DEFAULT_GOALS);
  const goals = (weeklyGoals && Object.keys(weeklyGoals).length>0) ? weeklyGoals : storedGoals;

  const updateGoals = (newGoals) => {
    storage.set("weeklyGoals", newGoals);
    if(setWeeklyGoals) setWeeklyGoals(newGoals);
  };
  ...
  // Build muscle map from all known exercises
  // Build muscle map including aliases for old exercise names
  const muscleMap = {};
  getAllExercises(customExercises).forEach(ex => { muscleMap[ex.name] = ex.muscle; });
  // Add aliases so old history entries are counted correctly
  Object.entries(EXERCISE_ALIASES).forEach(([oldName, {muscle}]) => {
    muscleMap[oldName] = muscle;
  });

  // Count ACTUAL series this week per muscle
  const muscleSets = {};
  history
    .filter(e => weekDays.includes(e.date))
    .forEach(e => {
      const muscle = muscleMap[e.exercise];
      if(muscle) muscleSets[muscle] = (muscleSets[muscle]||0) + (Number(e.sets)||3);
    });
  ...
  return (
    <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px 8px"}}>
        <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:1.5,color:"var(--muted)",fontWeight:600}}>Tydzień – serie na partię ciała</div>
        <div style={{fontSize:11,color:"var(--muted)"}}>{wStart} – {wEndStr}</div>
      </div>
      <div className="card" style={{marginBottom:12}}>
        {muscles.map(m => {
          const done = muscleSets[m]||0;
          const goal = goals[m] || DEFAULT_GOALS[m] || 9;
          const pct  = Math.min(100, Math.round((done/goal)*100));
          const col  = MUSCLE_COLORS[m]||"#666";
          return (
            <div key={m} style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{MUSCLE_LABELS[m]}</span>
                <span style={{fontSize:13,color:done>=goal?"#22c55e":"var(--muted)"}}>
                  <strong style={{color:col,fontFamily:"'Bebas Neue'",fontSize:18}}>{done}</strong>
                  <span style={{fontSize:11}}> / {goal} serii</span>
                </span>
              </div>
              <div style={{background:"var(--border)",borderRadius:6,height:10,position:"relative"}}>
                <div style={{height:"100%",borderRadius:6,background:done>=goal?"#22c55e":col,width:`${pct}%`,transition:"width .4s"}}/>
              </div>
            </div>
          );
        })}
        <button
          onClick={()=>{ setTempGoals({...goals}); setEditGoals(true); }}
          style={{width:"100%",marginTop:4,padding:"12px",borderRadius:12,border:"1.5px solid #ef4444",background:"#ef444411",color:"#ef4444",fontSize:14,fontWeight:700,cursor:"pointer"}}>
          🎯 Zmień cele serii
        </button>
      </div>
      ...
      {editGoals && (
        <div className="modal-backdrop" onClick={()=>setEditGoals(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {muscles.map(m => {
              const val = tempGoals[m] !== undefined ? tempGoals[m] : (goals[m]||DEFAULT_GOALS[m]||9);
              return (
                <div key={m} style={{marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{fontSize:14,color:MUSCLE_COLORS[m],fontWeight:700}}>{MUSCLE_LABELS[m]}</span>
                    <span style={{fontFamily:"'Bebas Neue'",fontSize:26,color:MUSCLE_COLORS[m]}}>{val}</span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <button
                      onClick={()=>setTempGoals(p=>({...p,[m]:Math.max(1,val-1)}))}
                      style={{width:44,height:44,borderRadius:12,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:26,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>−</button>
                    <div style={{flex:1,background:"var(--border)",borderRadius:5,height:8}}>
                      <div style={{height:"100%",borderRadius:5,background:MUSCLE_COLORS[m],width:`${Math.min(100,Math.round((val/20)*100))}%`,transition:"width .2s"}}/>
                    </div>
                    <button
                      onClick={()=>setTempGoals(p=>({...p,[m]:val+1}))}
                      style={{width:44,height:44,borderRadius:12,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:26,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>+</button>
                  </div>
                </div>
              );
            ...
            <button className="btn btn-primary" style={{marginTop:16}}
              onClick={()=>{ updateGoals({...DEFAULT_GOALS,...tempGoals}); setEditGoals(false); }}>
              ✓ Zapisz cele
            </button>
...
// ── SCREEN: KALENDARZ ─────────────────────────────────────────────────────────
function ScreenCalendar({calDate,setCalDate,workoutDates,dayLogs,history,selDay,setSelDay,customExercises=[],weeklyGoals,setWeeklyGoals}) {
  const {y,m}=calDate;
  const todayStr=today();
  ...
  const getSeriesInfo=ds=>{
    ...
    const muscleMapLocal={};
    getAllExercises(customExercises).forEach(ex=>{muscleMapLocal[ex.name]=ex.muscle;});
    Object.entries(EXERCISE_ALIASES).forEach(([old,{muscle}])=>{muscleMapLocal[old]=muscle;});
    const byMuscle={};
    entries.forEach(e=>{
      const muscle=muscleMapLocal[e.exercise];
      const sets=e.sets||3;
      if(muscle) { byMuscle[muscle]=(byMuscle[muscle]||0)+sets; }
      byMuscle[e.type]=(byMuscle[e.type]||0)+sets;
    });
    return byMuscle;
  };
  ...
  return (
    <div className="animate-up">
      ...
      {/* WEEKLY MUSCLE GROUPS - uses shared weeklyGoals from App state */}
...
function ScreenStats({history, dayLogs, customExercises=[]}) {
  ...
  // All known exercises (base + custom + discovered from history)
  const allKnownEx = (() => {
    const base = getAllExercises(customExercises);
    const known = new Set(base.map(e=>e.name));
    const extra = [];
    history.forEach(e=>{
      if(!known.has(e.exercise)){
        known.add(e.exercise);
        extra.push({id:`h_${e.exercise}`,name:e.exercise,muscle:"klata"});
      }
    });
    return [...base,...extra];
  })();

  // Get exercises for a group (push/pull/fbw or muscle name)
  const getExForGroup = (group) => {
    if(["push","pull","fbw"].includes(group)){
      const musclesInGroup = {
        push:["klata","barki","triceps"],
        pull:["plecy","biceps"],
        fbw: MUSCLE_GROUPS
      }[group];
      const fromBase = [...new Set((EXERCISES[group]||[]).map(e=>e.name))];
      const fromCustom = allKnownEx
        .filter(e=>musclesInGroup.includes(e.muscle) && !fromBase.includes(e.name))
        .map(e=>e.name);
  ...
  // History for selected exercise - also include old alias names
  const selExerciseAliases = new Set([selExercise]);
  Object.entries(EXERCISE_ALIASES).forEach(([oldName,{name}])=>{
    if(name===selExercise) selExerciseAliases.add(oldName);
    if(oldName===selExercise) selExerciseAliases.add(name);
  });
  const exHistory = history
    .filter(e => selExerciseAliases.has(e.exercise))
  ...
  const getLastMuscle = (muscle) => {
    const names = new Set(allKnownEx.filter(e=>e.muscle===muscle).map(e=>e.name));
    ...
    return {date:lastDate, entries:[...new Set(entries.filter(e=>e.date===lastDate).map(e=>e.exercise))].map(exName=>entries.find(e=>e.date===lastDate&&e.exercise===exName))};
  };

  // Best per exercise for muscle
  const getBestPerEx = (muscle) => {
    const names = new Set(allKnownEx.filter(e=>e.muscle===muscle).map(e=>e.name));
    // Also add canonical names for aliased exercises in this muscle
    Object.entries(EXERCISE_ALIASES).forEach(([,{name:cn,muscle:m}])=>{ if(m===muscle) names.add(cn); });
    return [...names].map(exName=>{
      // Include entries with old alias names
      const aliases = new Set([exName]);
      Object.entries(EXERCISE_ALIASES).forEach(([old,{name}])=>{ if(name===exName) aliases.add(old); if(old===exName) aliases.add(name); });
      const entries = history.filter(e=>aliases.has(e.exercise)).sort((a,b)=>new Date(b.date)-new Date(a.date));
      if(!entries.length) return null;
      return {
        name:exName,
        latest:entries[0].weight,
        best:Math.max(...entries.map(e=>e.weight)),
        date:entries[0].date,
        sets:entries[0].sets||1,
      };
    }).filter(Boolean);
  };
  ...
  const BODY_PARTS=[
    {key:"klatka",label:"Klatka",x:50,y:26},{key:"biceps_l",label:"Biceps L",x:20,y:35},
    {key:"biceps_p",label:"Biceps P",x:80,y:35},{key:"pas",label:"Pas",x:50,y:50},
    {key:"biodra",label:"Biodra",x:50,y:60},{key:"udo_l",label:"Udo L",x:30,y:72},
    {key:"udo_p",label:"Udo P",x:70,y:72},{key:"lydka_l",label:"Łydka L",x:30,y:87},
    {key:"lydka_p",label:"Łydka P",x:70,y:87}
  ];
    ...
    <div className="animate-up">
      ...
      <div style={{display:"flex",gap:6,margin:"0 16px 12px",overflowX:"auto"}}>
        {[
          ...
          {id:"muscle_last",label:"💪 Partie"},
          {id:"body",       label:"⚖️ Sylwetka"},
          {id:"week",       label:"📅 Tygodnie"},
        ].map(t=>(
      ...
      {activeTab==="exercises" && <>
        ...
        <div style={{margin:"0 16px 8px",overflowX:"auto",display:"flex",gap:6,paddingBottom:4}}>
          {groupTabs.map(g=>(
            <button key={g.id}
              onClick={()=>{
                setSelGroup(g.id);
                const exs = getExForGroup(g.id);
                if(exs.length) setSelExercise(exs[0]);
              }}
          ...
          {exListForGroup.map(ex=>(
            <button key={ex} onClick={()=>setSelExercise(ex)}
              style={{flexShrink:0,border:"1px solid var(--border)",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:11,
                background:selExercise===ex?tc[selGroup]+"22":"var(--card2)",
                color:selExercise===ex?tc[selGroup]:"var(--muted)",
                outline:selExercise===ex?`1.5px solid ${tc[selGroup]}44`:"none"}}>
              {ex}
            </button>
          ))}
        </div>
        ...
        <div className="card">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:12}}>
            ...
            {exHistory.length>0 && (
              <div style={{textAlign:"right"}}>
                <div style={{fontFamily:"'Bebas Neue'",fontSize:28,color:tc[selGroup]||"#ef4444"}}>{maxW} kg</div>
          ...
          {exHistory.length>1 ? (
            <svg width="100%" height={chartH+30} viewBox={`0 0 100 ${chartH+30}`} preserveAspectRatio="none" style={{overflow:"visible"}}>
              <defs>
                <linearGradient id="grad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={tc[selGroup]||"#ef4444"} stopOpacity=".3"/>
                  <stop offset="100%" stopColor={tc[selGroup]||"#ef4444"} stopOpacity="0"/>
                </linearGradient>
              </defs>
              <path d={[
                `M 0 ${chartH-((exHistory[0].weight-minW)/range)*chartH}`,
                ...exHistory.slice(1).map((_,i)=>`L ${((i+1)/pts)*100} ${chartH-((exHistory[i+1].weight-minW)/range)*chartH}`),
                `L 100 ${chartH}`,`L 0 ${chartH}`,"Z"
              ].join(" ")} fill="url(#grad)"/>
              <polyline
                points={exHistory.map((e,i)=>`${(i/pts)*100},${chartH-((e.weight-minW)/range)*chartH}`).join(" ")}
                fill="none" stroke={tc[selGroup]||"#ef4444"} strokeWidth="1.5"/>
              {exHistory.map((e,i)=>(
                <circle key={i} cx={(i/pts)*100} cy={chartH-((e.weight-minW)/range)*chartH} r="2.5" fill={tc[selGroup]||"#ef4444"}/>
              ))}
            ...
            <div style={{textAlign:"center",padding:"16px 0",color:"var(--muted)",fontSize:13}}>Jeden pomiar: {exHistory[0].weight} kg ({fmt(exHistory[0].date)})</div>
          ) : (
            <div style={{textAlign:"center",padding:"16px 0",color:"var(--muted)",fontSize:13}}>Brak danych dla tego ćwiczenia</div>
          )}
        </div>
        {exHistory.length>0 && (
          <div className="card">
              ...
              return (
                <div key={i} className="ex-row">
                  <div>
                    ...
                    <div style={{fontSize:10,color:"var(--muted)",marginTop:1}}>{e.type?.toUpperCase()} · {e.sets||3} serii</div>
                  </div>
      ...
      {activeTab==="body" && <>
        ...
        <div className="card">
          ...
          {weightSorted.length>0 && [...weightSorted].reverse().slice(0,8).map((w,i,arr)=>{
      ...
      {editMeas && (()=>{
        ...
        return (
          <div className="modal-backdrop" onClick={()=>setEditMeas(null)}>
            <div className="modal" onClick={e=>e.stopPropagation()}>
              <div className="modal-handle"/>
              ...
              <button className="btn btn-primary" onClick={()=>{setMeasurements(tempMeas);setEditMeas(null);}}>Zapisz</button>
              {measurements[editMeas] && (
                <button className="btn btn-ghost" style={{width:"100%",marginTop:8,color:"#ef4444"}}
                  onClick={()=>{const m={...measurements};delete m[editMeas];setMeasurements(m);setEditMeas(null);}}>
                  Usuń pomiar
                </button>
              )}
