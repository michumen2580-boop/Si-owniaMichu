      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginTop:16,marginBottom:8}}>
          ...
          {Object.entries({klata:"Klatka",plecy:"Plecy",barki:"Barki",biceps:"Biceps",triceps:"Triceps",nogi:"Nogi"}).map(([m,label])=>{
            const MUSCLE_COLORS_S = {klata:"#ef4444",plecy:"#3b82f6",barki:"#f97316",biceps:"#8b5cf6",triceps:"#eab308",nogi:"#22c55e"};
            ...
            return (
              <div key={m} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid var(--border)"}}>
                <span style={{fontSize:14,color:MUSCLE_COLORS_S[m],fontWeight:600}}>{label}</span>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <button onClick={()=>setWeeklyGoals(p=>({...p,[m]:Math.max(1,val-1)}))}
                    style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                  <span style={{fontFamily:"'Bebas Neue'",fontSize:22,minWidth:28,textAlign:"center",color:MUSCLE_COLORS_S[m]}}>{val}</span>
                  <button onClick={()=>setWeeklyGoals(p=>({...p,[m]:val+1}))}
                    style={{width:36,height:36,borderRadius:10,border:"1px solid var(--border)",background:"var(--card2)",color:"var(--text)",fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                </div>
              </div>
            );
...
function SessionView({type,history,setHistory,todayStr,onBack,saveDay,settings,customExercises=[]}) {
  ...
  const handleFinish=()=>{
    const newEntries=exList.filter(ex=>weights[ex.id]).map(ex=>({
      ...
      sets:(done[ex.id]||[]).filter(Boolean).length || SETS,
...
function WeeklyMuscleProgress({history, customExercises=[], weeklyGoals, setWeeklyGoals}) {
  ...
  // Always use external goals from App state
  const goals = (weeklyGoals && Object.keys(weeklyGoals).length>0) ? weeklyGoals : DEFAULT_GOALS;
  ...
  history
    ...
    .forEach(e => {
      ...
      if(muscle) muscleSets[muscle] = (muscleSets[muscle]||0) + (Number(e.sets)||1);
    });
              ...
              return (
                <div key={i} className="ex-row">
                  <div>
                    ...
                    <div style={{fontSize:10,color:"var(--muted)",marginTop:1}}>{e.type?.toUpperCase()} · {e.sets||1} serie</div>
                  </div>
