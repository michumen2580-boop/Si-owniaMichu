  ...
  return (
    <div className="anim">
      ...
      {(()=>{
        ...
        const totalTarget   = goalKcal * 7;   // cel (co chcesz zjeść)
        const totalTdee     = tdee * 7;        // zapotrzebowanie (co spalasz)
        const deficit       = totalTdee - totalEaten; // vs TDEE: ile naprawdę tracisz/zyskujesz
        const vsGoal        = totalTarget - totalEaten; // vs cel: czy trzymasz plan
        const fatKg         = (Math.abs(deficit) / 7000).toFixed(2);
        const isDeficit     = deficit >= 0;
        const onTrack       = vsGoal >= 0; // czy jesz poniżej celu
        ...
        return (
          <div className="card" style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:12}}>
              <div>
                <div style={{fontFamily:"'Bebas Neue'",fontSize:36,color:isDeficit?"#22c55e":"#ef4444",lineHeight:1}}>
                  {isDeficit?"-":"+"}{Math.abs(deficit).toLocaleString()} kcal
                </div>
                <div style={{fontSize:12,color:"var(--muted)",marginTop:4}}>
                  {isDeficit?"Deficyt":"Nadwyżka"} vs TDEE ({tdee} kcal/dzień)
                </div>
                <div style={{fontSize:11,color:onTrack?"#22c55e":"#ef4444",marginTop:2}}>
                  {onTrack?"✓ Poniżej celu":"✗ Powyżej celu"} · zjedzone {totalEaten.toLocaleString()} / {totalTarget.toLocaleString()} kcal
                </div>
              </div>
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontSize:11,color:"var(--muted)"}}>tłuszczu (est.)</div>
              </div>
            </div>
            <div style={{background:"var(--border)",borderRadius:6,height:10,marginBottom:8}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"var(--muted)"}}>
              <span>0 kcal</span>
              <span style={{color:pct>100?"#ef4444":"var(--muted)"}}>{pct}% celu</span>
              <span>{totalTarget.toLocaleString()} kcal</span>
            </div>
            <div style={{marginTop:10,padding:"8px 12px",background:"var(--card2)",borderRadius:10,fontSize:12,color:"var(--muted2)"}}>
              💡 7 000 kcal deficytu ≈ 1 kg tłuszczu{" "}·{" "}
              deficyt dzienny od TDEE:{" "}
              <strong style={{color:isDeficit?"#22c55e":"#ef4444"}}>
                {tdee-goalKcal > 0 ? `-${tdee-goalKcal}` : `+${Math.abs(tdee-goalKcal)}`} kcal/dzień
              </strong>
            </div>
          </div>
        );
