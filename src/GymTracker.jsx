...
const NEW_FBW = [
  ...
  // NEW from PDF 30.05
  {date:"2026-05-30",type:"fbw",exercise:"Płaska",weight:37},
  {date:"2026-05-30",type:"fbw",exercise:"Skos",weight:30},
  {date:"2026-05-30",type:"fbw",exercise:"Barki góra",weight:22},
  {date:"2026-05-30",type:"fbw",exercise:"Ściąganie góra szeroko",weight:80},
  {date:"2026-05-30",type:"fbw",exercise:"Ściąganie góra wąsko",weight:80},
  {date:"2026-05-30",type:"fbw",exercise:"Biceps skos",weight:17},
  {date:"2026-05-30",type:"fbw",exercise:"Młotki",weight:15},
  {date:"2026-05-30",type:"fbw",exercise:"Triceps dół",weight:35},
...
const SEED_PUSH_PULL = [
  ...
  {date:"2026-02-04",type:"pull",exercise:"Biceps skos",weight:17.5},
...
const typeColor = {push:"#ef4444",pull:"#3b82f6",fbw:"#22c55e",cardio:"#eab308",work:"#4b5563",training:"#ef4444",recovery:"#22c55e",rest:"#374151"};
...
export default function App() {
  ...
  const [history, setHistory]   = useState(() => { const s=storage.get("gymHistory",null); if(s) return s; storage.set("gymHistory",SEED_HISTORY); return SEED_HISTORY; });
    ...
    <>
      ...
      <div className="app">
        <div className="screen">
          ...
          {tab==="training" && <ScreenTraining history={history} setHistory={setHistory} saveDay={saveDay} todayStr={todayStr} dayLogs={dayLogs} settings={settings}/>}
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginTop:16,display:"flex",flexDirection:"column",gap:10}}>
          <button className="btn btn-ghost" style={{width:"100%"}} onClick={()=>{
            ...
            const a = document.createElement("a"); a.href=url; a.download="gymtracker-backup.json"; a.click();
...
// ── SCREEN: TRENING ───────────────────────────────────────────────────────────
function ScreenTraining({history,setHistory,saveDay,todayStr,dayLogs,settings}) {
  ...
  if(phase==="session") return <SessionView type={selType} history={history} setHistory={setHistory} todayStr={todayStr} onBack={()=>setPhase("select")} saveDay={saveDay} settings={settings}/>;
        ...
        return dates.length?(
          <div key={type} className="card" style={{marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              <span style={{fontFamily:"'Bebas Neue'",fontSize:18,color:tColors[type]}}>{type.toUpperCase()}</span>
            </div>
            {dates.map(d=>(
...
// ── SESSION VIEW ──────────────────────────────────────────────────────────────
function SessionView({type,history,setHistory,todayStr,onBack,saveDay,settings}) {
  const SETS = 3;
  ...
  const getLastWeight=useCallback((exName)=>{
    const m=history.filter(e=>e.exercise===exName&&e.type===type).sort((a,b)=>a.date>b.date?-1:1);
  ...
  const handleFinish=()=>{
    const newEntries=exList.filter(ex=>weights[ex.id]).map(ex=>({date:todayStr,type,exercise:ex.name,weight:parseFloat(weights[ex.id]),sets:SETS}));
    if(newEntries.length) setHistory(prev=>[...prev,...newEntries]);
  ...
  return (
    <div className="animate-up">
      ...
      {exList.map(ex=>{
        ...
        return (
          <div key={ex.id} className="card" style={{opacity:exDone?.85:1,transition:"opacity .2s"}}>
              ...
              <div key={key}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  <input type="number" placeholder="0" value={newMeal[key]} onChange={e=>setNewMeal(p=>({...p,[key]:e.target.value}))}
                    style={{flex:1,background:"var(--card2)",border:"1px solid var(--border)",borderRadius:8,padding:"8px 10px",color:"var(--text)",fontFamily:"'DM Sans'",fontSize:14,outline:"none",textAlign:"right"}}/>
...
function ScreenCalendar({calDate,setCalDate,workoutDates,dayLogs,history,selDay,setSelDay}) {
  ...
  return (
    <div className="animate-up">
      ...
      {/* MONTH SUMMARY */}
      <div className="section-label">Podsumowanie – serie na partie</div>
...
function ScreenStats({history,dayLogs}) {
  ...
  const allExercises={push:[...new Set(EXERCISES.push.map(e=>e.name))],pull:[...new Set(EXERCISES.pull.map(e=>e.name))],fbw:[...new Set(EXERCISES.fbw.map(e=>e.name))]};
  const exHistory=history.filter(e=>e.exercise===selExercise).sort((a,b)=>a.date>b.date?1:-1);
  ...
  const BODY_PARTS=[{key:"klatka",label:"Klatka",x:"45%",y:"28%"},{key:"biceps_l",label:"Biceps L",x:"18%",y:"35%"},{key:"biceps_p",label:"Biceps P",x:"75%",y:"35%"},{key:"pas",label:"Pas",x:"45%",y:"50%"},{key:"biodra",label:"Biodra",x:"45%",y:"60%"},{key:"udo_l",label:"Udo L",x:"30%",y:"72%"},{key:"udo_p",label:"Udo P",x:"60%",y:"72%"},{key:"lydka_l",label:"Łydka L",x:"30%",y:"88%"},{key:"lydka_p",label:"Łydka P",x:"60%",y:"88%"}];

  const tc={push:"#ef4444",pull:"#3b82f6",fbw:"#22c55e"};
    ...
    <div className="animate-up">
      ...
      <div style={{display:"flex",gap:6,margin:"0 16px 12px",overflowX:"auto"}}>
        {Object.entries(totals).map(([tp,n])=>(
          <div key={tp} style={{flexShrink:0,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"12px 14px",textAlign:"center",minWidth:68}}>
        ...
        <div style={{flexShrink:0,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"12px 14px",textAlign:"center",minWidth:68}}>
        ...
        <div style={{flexShrink:0,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,padding:"12px 14px",textAlign:"center",minWidth:68}}>
          <div style={{fontFamily:"'Bebas Neue'",fontSize:24,color:"#f97316"}}>{Math.round(totalKcalActive/1000)}k</div>
          <div style={{fontSize:9,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.8}}>Kcal</div>
        </div>
      </div>

      <div style={{display:"flex",gap:6,margin:"0 16px 12px",overflowX:"auto"}}>
        {[{id:"exercises",label:"Ćwiczenia"},{id:"body",label:"Sylwetka"},{id:"week",label:"Tygodnie"}].map(t=>(
      ...
      {/* TAB EXERCISES */}
      {activeTab==="exercises"&&<>
        <div style={{display:"flex",gap:6,margin:"0 16px 10px",overflowX:"auto",paddingBottom:2}}>
          {Object.keys(allExercises).map(tp=>(
            <button key={tp} onClick={()=>setSelType(tp)}
              style={{flexShrink:0,border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontWeight:700,fontSize:12,background:selType===tp?tc[tp]+"33":"var(--card2)",color:selType===tp?tc[tp]:"var(--muted)",outline:selType===tp?`1.5px solid ${tc[tp]}`:"none"}}>
              {tp.toUpperCase()}
            </button>
          ))}
        </div>
        <div style={{margin:"0 16px 8px",overflowX:"auto",display:"flex",gap:6,paddingBottom:4}}>
          {allExercises[selType].map(ex=>(
            <button key={ex} onClick={()=>setSelExercise(ex)}
              style={{flexShrink:0,border:"1px solid var(--border)",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:11,background:selExercise===ex?tc[selType]+"22":"var(--card2)",color:selExercise===ex?tc[selType]:"var(--muted)",outline:selExercise===ex?`1.5px solid ${tc[selType]}44`:"none"}}>
              {ex}
            </button>
          ))}
        </div>
        <div className="card">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:12}}>
            ...
            {exHistory.length>0&&<div style={{textAlign:"right"}}><div style={{fontFamily:"'Bebas Neue'",fontSize:28,color:tc[selType]}}>{maxW} kg</div><div style={{fontSize:10,color:"var(--muted)"}}>REKORD</div></div>}
          </div>
          {exHistory.length>1?(
            <svg width="100%" height={chartH+30} viewBox={`0 0 100 ${chartH+30}`} preserveAspectRatio="none" style={{overflow:"visible"}}>
              <defs><linearGradient id="grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={tc[selType]} stopOpacity=".3"/><stop offset="100%" stopColor={tc[selType]} stopOpacity="0"/></linearGradient></defs>
              ...
              <polyline points={exHistory.map((e,i)=>`${(i/chartW_pts)*100},${chartH-((e.weight-minW)/range)*chartH}`).join(" ")} fill="none" stroke={tc[selType]} strokeWidth="1.5"/>
              {exHistory.map((e,i)=><circle key={i} cx={(i/chartW_pts)*100} cy={chartH-((e.weight-minW)/range)*chartH} r="2" fill={tc[selType]}/>)}
        ...
        {exHistory.length>0&&(
          <div className="card">
            ...
            {[...exHistory].reverse().slice(0,10).map((e,i,arr)=>{ const prev=arr[i+1]; const diff=prev?e.weight-prev.weight:0; return (
              <div key={i} className="ex-row">
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <span className="ex-weight" style={{color:tc[selType]}}>{e.weight} <span style={{fontSize:12}}>kg</span></span>
                </div>
              </div>
            );})}
          </div>
      ...
      {activeTab==="body"&&<>
        ...
        <div className="card">
          ...
          <div style={{position:"relative",width:"100%",paddingBottom:"120%"}}>
            <svg viewBox="0 0 100 160" style={{position:"absolute",inset:0,width:"100%",height:"100%"}}>
              {/* simple body outline */}
              <ellipse cx="50" cy="12" rx="10" ry="11" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="35" y="22" width="30" height="40" rx="6" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="18" y="24" width="14" height="30" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="68" y="24" width="14" height="30" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="34" y="62" width="14" height="45" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="52" y="62" width="14" height="45" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="33" y="106" width="13" height="35" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              <rect x="54" y="106" width="13" height="35" rx="5" fill="#222" stroke="#444" strokeWidth="1"/>
              {BODY_PARTS.map(bp=>{
                const val=measurements[bp.key];
                return (
                  <g key={bp.key} onClick={()=>{ setTempMeas({...measurements}); setEditMeas(bp.key); }} style={{cursor:"pointer"}}>
                    <circle cx={parseFloat(bp.x)} cy={parseFloat(bp.y)*1.6} r="5" fill={val?"#ef444488":"#33333388"} stroke={val?"#ef4444":"#555"} strokeWidth="0.8"/>
                    <text x={parseFloat(bp.x)} y={parseFloat(bp.y)*1.6+0.5} textAnchor="middle" dominantBaseline="middle" fontSize="3.5" fill={val?"#ef4444":"#888"} fontWeight="bold">{val?`${val}`:"+"}</text>
                  </g>
                );
              })}
            </svg>
        ...
        <div className="card">
          ...
          {weightSorted.length>0&&(
            <>
              <div style={{marginTop:12,display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:8}}>
                <div style={{fontSize:12,color:"var(--muted)"}}>{weightSorted.length} pomiarów</div>
              ...
              <div className="card-title" style={{marginTop:12}}>Historia wagi</div>
        ...
        return (
          <div className="modal-backdrop" onClick={()=>setEditMeas(null)}>
            <div className="modal" onClick={e=>e.stopPropagation()}>
              ...
              {measurements[editMeas]&&<button className="btn btn-ghost" style={{width:"100%",marginTop:8,color:"#ef4444"}} onClick={()=>{ const m={...measurements}; delete m[editMeas]; setMeasurements(m); setEditMeas(null); }}>Usuń pomiar</button>}
