...
function ScreenDiet({saveDay}) {
  ...
  const [aiMode,    setAiMode]    = useState(false);   // true = show AI text input
  const [aiDesc,    setAiDesc]    = useState("");       // AI text description
  ...
  const handleImg=(e)=>{ const f=e.target.files[0]; if(!f)return; const r=new FileReader(); r.onload=(ev)=>{ setImgPrev(ev.target.result); analyzeImage(ev.target.result); }; r.readAsDataURL(f); };
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:[
            ...
            {type:"text",text:`Oszacuj wartości odżywcze posiłku na zdjęciu. Podaj standardową porcję jeśli nie widać ilości. Odpowiedz TYLKO JSON bez żadnego tekstu: {"name":"nazwa posiłku po polsku","kcal":0,"protein":0,"carbs":0,"fat":0}`}
          ]}]
        })
      });
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          model:"claude-sonnet-4-20250514",
          max_tokens:300,
          messages:[{role:"user",content:`Oszacuj wartości odżywcze: "${desc}". Standardowa porcja jeśli nie podano ilości. Odpowiedz TYLKO JSON: {"name":"nazwa po polsku","kcal":0,"protein":0,"carbs":0,"fat":0}`}]
        })
      });
      const json = await res.json();
      const text = json.content?.[0]?.text||"";
        ...
        <div className="card">
          ...
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            ...
            <button onClick={()=>setAiMode(m=>!m)}
              style={{flex:1,background:aiMode?"#7c3aed22":"var(--card2)",border:`1.5px dashed ${aiMode?"#7c3aed":"var(--border)"}`,borderRadius:10,padding:10,display:"flex",flexDirection:"column",alignItems:"center",gap:4,cursor:"pointer",color:aiMode?"#a78bfa":"var(--muted)"}}>
              <span style={{fontSize:20}}>🤖</span><span style={{fontSize:10}}>AI</span>
            </button>
          </div>
          ...
          {aiMode&&!aiLoading&&(
            <div style={{marginBottom:8,background:"#7c3aed22",borderRadius:10,padding:10,border:"1px solid #7c3aed44"}}>
              <div style={{fontSize:11,color:"#a78bfa",marginBottom:6,fontWeight:600}}>🤖 Opisz posiłek:</div>
              <textarea
                placeholder="np. kurczak z ryżem 200g, owsianka z owocami, bułka z serem i szynką..."
                value={aiDesc}
                onChange={e=>setAiDesc(e.target.value)}
                style={{width:"100%",minHeight:60,background:"var(--card2)",border:"1px solid #7c3aed44",borderRadius:8,padding:"8px 10px",color:"var(--text)",fontFamily:"'DM Sans'",fontSize:13,outline:"none",resize:"none",boxSizing:"border-box"}}
              />
              <button onClick={()=>analyzeText(aiDesc)}
                style={{width:"100%",marginTop:6,padding:"10px",borderRadius:8,border:"none",background:"#7c3aed",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>
                🤖 Analizuj
              </button>
            </div>
          )}
