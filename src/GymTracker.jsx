  ...
  return (
    <>
      ...
      <div className={`app ${settings.darkMode?"":"light"}`}>
        ...
        {showSettings && <SettingsModal settings={settings} setSettings={setSettings} weeklyGoals={weeklyGoals} setWeeklyGoals={setWeeklyGoals} history={history} dayLogs={dayLogs} exerciseDB={exerciseDB} onClose={()=>setShowSettings(false)}/>}
      </div>
    </>
  );
}
...
// ── SETTINGS MODAL ────────────────────────────────────────────────────────────
function SettingsModal({settings,setSettings,weeklyGoals,setWeeklyGoals,history,dayLogs,exerciseDB={},onClose}) {
  ...
  // One column per date, value = "3×100kg" or empty
  ...
  const rows = allNames.map(name=>{
    ...
    const histMap = {};
    (ex.history||[]).forEach(h=>{ histMap[h.date]=`${h.sets||3}×${h.weight}kg`; });
  ...
  return (
    <div className="anim">
      ...
      {(()=>{
        ...
        const totalTarget = goalKcal * 7;          // pula tygodniowa wg celu
        const remaining   = totalTarget - totalEaten; // ile zostało do zjedzenia
        const pct         = Math.min(100, Math.round((totalEaten / totalTarget) * 100));
        const onTrack     = remaining >= 0;
        // Rzeczywisty deficyt vs TDEE (ile naprawdę tracisz)
        const realDeficit = tdee * 7 - totalEaten;
        const fatKg       = (Math.abs(realDeficit) / 7000).toFixed(2);
        const today       = week7[week7.length-1];
        const daysDone    = week7.filter(d=>d<=today).length;
        const daysLeft    = 7 - daysDone;
        const kcalPerDay  = daysLeft > 0 ? Math.round(remaining / daysLeft) : 0;
        return (
          <div className="card" style={{marginBottom:12}}>
            <div className="ctitle">Bilans tygodniowy · pula {totalTarget.toLocaleString()} kcal</div>

            {/* GŁÓWNA LICZBA - ile zostało */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div>
                <div style={{fontFamily:"'Bebas Neue'",fontSize:42,lineHeight:1,color:onTrack?"#22c55e":"#ef4444"}}>
                  ...
                  <span style={{fontSize:18}}> kcal</span>
                </div>
                <div style={{fontSize:13,color:"var(--muted)",marginTop:4}}>
                  {onTrack?"pozostało do zjedzenia":"przekroczono pulę"}
                </div>
                {daysLeft>0&&<div style={{fontSize:11,color:"var(--muted2)",marginTop:2}}>
                  ≈ <strong style={{color:onTrack?"#22c55e":"#ef4444"}}>{kcalPerDay} kcal/dzień</strong> przez {daysLeft} {daysLeft===1?"dzień":"dni"}
                </div>}
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontFamily:"'Bebas Neue'",fontSize:26,color:realDeficit>=0?"#22c55e":"#ef4444"}}>
                  {realDeficit>=0?"-":"+"}{fatKg} kg
                </div>
                <div style={{fontSize:10,color:"var(--muted)"}}>tłuszczu (est.)</div>
                <div style={{fontSize:10,color:"var(--muted)",marginTop:2}}>vs TDEE {tdee} kcal</div>
              </div>
            </div>

            {/* PASEK POSTĘPU */}
            <div style={{background:"var(--border)",borderRadius:6,height:12,marginBottom:6,position:"relative",overflow:"hidden"}}>
              <div style={{height:"100%",borderRadius:6,background:pct>100?"#ef4444":"#22c55e",width:`${Math.min(100,pct)}%`,transition:"width .4s"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"var(--muted)",marginBottom:12}}>
              <span>zjedzone: <strong style={{color:"var(--text)"}}>{totalEaten.toLocaleString()}</strong></span>
              <span style={{color:pct>100?"#ef4444":"var(--muted2)",fontWeight:600}}>{pct}%</span>
              <span>pula: <strong style={{color:"var(--text)"}}>{totalTarget.toLocaleString()}</strong></span>
            </div>

            {/* PODPOWIEDŹ */}
            <div style={{padding:"8px 12px",background:"var(--card2)",borderRadius:10,fontSize:12,color:"var(--muted2)"}}>
              💡 Plan: -{tdee-goalKcal} kcal/dzień deficytu · szacowany efekt tygodniowy:{" "}
              <strong style={{color:"#22c55e"}}>-{((tdee-goalKcal)*7/1000).toFixed(2)} kg</strong>{" "}
              · 7 000 kcal ≈ 1 kg tłuszczu
            </div>
          </div>
        );
      })()}
