...
function ScreenTraining({history,setHistory,saveDay,todayStr,dayLogs,settings}) {
  ...
  const saveEditEx = () => {
    if(!editEx) return;
    // Update in customExercises or add as override
    const existing = customExercises.find(e=>e.name===editEx.originalName);
    if(existing) {
      setCustomExercises(prev=>prev.map(e=>e.name===editEx.originalName?{...e,name:editEx.name,muscle:editEx.muscle}:e));
    } else {
      ...
      {editEx && (
        <div className="modal-backdrop" onClick={()=>setEditEx(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {customExercises.find(e=>e.name===editEx.originalName||e.editOf===editEx.originalName) && (
              <button className="btn btn-ghost" style={{width:"100%",marginTop:8,color:"#ef4444"}} onClick={()=>{deleteCustomEx(editEx.name);setEditEx(null);}}>Usuń ćwiczenie</button>
            )}
...
function WeeklyMuscleProgress({history}) {
  ...
  const muscleMap = {};
  [...EXERCISES.push,...EXERCISES.pull,...EXERCISES.fbw].forEach(ex=>{ muscleMap[ex.name]=ex.muscle; });
  const muscleSets = {};
...
function ScreenStats({history,dayLogs}) {
  ...
  const getExercisesForGroup = (group) => {
    if(["push","pull","fbw"].includes(group)) {
      return [...new Set(EXERCISES[group].map(e=>e.name))];
    }
    // muscle group - get all exercises with that muscle
    const allEx = getAllExercises([]);
    return [...new Set(allEx.filter(e=>e.muscle===group).map(e=>e.name))];
  };
          ...
          {MUSCLE_GROUPS.filter(m=>m!=="brzuch").map(muscle=>{
            ...
            return (
              <div key={muscle} className="card" style={{marginBottom:8,borderLeft:`3px solid ${MUSCLE_COLORS[muscle]}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:last?10:0}}>
                ...
                {last && last.entries.slice(0,4).map((e,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderTop:"1px solid var(--border)"}}>
                    <span style={{fontSize:12,color:"var(--muted2)"}}>{e.exercise}</span>
                    <span style={{fontFamily:"'Bebas Neue'",fontSize:16,color:MUSCLE_COLORS[muscle]}}>{e.weight}kg</span>
                  </div>
                ))}
                {last && last.entries.length>4 && <div style={{fontSize:11,color:"var(--muted)",marginTop:4}}>+{last.entries.length-4} więcej</div>}
              </div>
            );
          })}
        </div>
