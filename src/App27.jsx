...
function WeeklyMuscleProgress({history,customExercises=[],weeklyGoals:externalGoals,setWeeklyGoals:setExternalGoals}) {
  const DEFAULT_GOALS = {klata:9,barki:9,triceps:6,plecy:12,biceps:9,nogi:6};
  // Always use external goals from App state (passed via props) - never stale
  const goals = (externalGoals && Object.keys(externalGoals).length > 0)
    ? externalGoals
    : (storage.get("weeklyGoals",null) || DEFAULT_GOALS);
  const setGoals = (v) => {
    if(setExternalGoals) setExternalGoals(v);
    storage.set("weeklyGoals",v);
  };
  ...
  return (
    <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 20px",marginBottom:8}}>
        <div className="section-label" style={{margin:0}}>Tydzień – serie na partię ciała</div>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <button onClick={()=>{ setTempGoals({...goals}); setEditGoals(true); }}
          style={{width:"100%",marginTop:8,padding:"12px",borderRadius:12,border:"1.5px solid #ef4444",background:"#ef444411",color:"#ef4444",fontSize:14,fontWeight:700,cursor:"pointer",letterSpacing:.5}}>
          🎯 Zmień cele serii
        </button>
      </div>
        ...
        <div className="modal-backdrop" onClick={()=>setEditGoals(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <button className="btn btn-primary" style={{marginTop:16}} onClick={()=>{
              const merged = {...DEFAULT_GOALS,...tempGoals};
              setGoals(merged);
              setEditGoals(false);
            }}>✓ Zapisz cele</button>
