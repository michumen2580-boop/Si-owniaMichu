...
function SessionView({type,history,setHistory,todayStr,onBack,saveDay,settings,customExercises=[]}) {
  ...
  useEffect(()=>{ if(restTimer===null)return; if(restTimer<=0){setRestTimer(null);setRestEx(null);if(settings?.vibration)vibrate([200,100,200,100,200]);return;} const t=setTimeout(()=>setRestTimer(r=>r-1),1000); return()=>clearTimeout(t); },[restTimer,restEx,settings]);
  ...
  const handleFinish = () => {
    const entries = exList
      .filter(ex=>weights[ex.id||ex.name])
  ...
  return (
    <div className="anim">
      <div style={{padding:"48px 20px 12px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1,fontFamily:"'Bebas Neue'",fontSize:28,letterSpacing:2,color:color}}>{(MUSCLE_LABEL[type]||type).toUpperCase()}</div>
      ...
      {[...exList].sort((a,b)=>{
        ...
        return (
          <div key={key} className="card" style={{opacity:exDone?0.7:1,border:isResting?`1.5px solid ${color}`:undefined,transition:"opacity .3s"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
              <div><div style={{fontWeight:600,fontSize:14}}>{ex.name}</div><div style={{fontSize:11,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>{ex.muscle}</div></div>
            ...
            {[["kcal","🔥 Kalorie","kcal"],["protein","🥩 Białko","g"],["carbs","🍞 Węgle","g"],["fat","🧈 Tłuszcze","g"]].map(([key,label,unit])=>(
              <div key={key}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  <input type="number" placeholder="0" value={newMeal[key]} onChange={e=>setNewMeal(p=>({...p,[key]:e.target.value}))} style={{flex:1,minWidth:0,background:"var(--card2)",border:"1px solid var(--border)",borderRadius:8,padding:"8px 6px",color:"var(--text)",fontFamily:"'DM Sans'",fontSize:13,outline:"none",textAlign:"right"}}/>
