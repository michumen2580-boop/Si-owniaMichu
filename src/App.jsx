...
const getMuscle = (exName, customExercises=[], exerciseDB={}) => {
...
const getExercisesForSession = (type, customExercises=[], hiddenExercises=[], workoutTemplates={}) => {
  const template = workoutTemplates[type]; // null = all, array of names = custom selection
  let base;
  if(EXERCISES[type]) {
    base = EXERCISES[type];
  } else {
    // Muscle-based session
    const allBase = Object.values(EXERCISES).flat().filter(e=>e.muscle===type);
    const seen = new Set(allBase.map(e=>e.name));
    const custom = customExercises.filter(e=>e.muscle===type&&!seen.has(e.name));
    base = [...allBase, ...custom];
  }
  // Add custom exercises for push/pull/fbw types
  if(EXERCISES[type]) {
    const seen = new Set(base.map(e=>e.name));
    const custom = customExercises.filter(e=>!seen.has(e.name)&&!e.isOverride);
    // Only add custom exercises that are in the template or if no template
    if(template) {
      return [...base.filter(e=>template.includes(e.name)), ...custom.filter(e=>template.includes(e.name))].filter(e=>!hiddenExercises.includes(e.name));
    }
    return base.filter(e=>!hiddenExercises.includes(e.name));
  }
  // Filter by template if set
  if(template) return base.filter(e=>template.includes(e.name)&&!hiddenExercises.includes(e.name));
  return base.filter(e=>!hiddenExercises.includes(e.name));
};
...
export default function App() {
  ...
  const [exerciseDB, setExerciseDB] = useState(()=>{
    ...
    if(ver < DATA_VERSION) {
      // First run or upgrade: merge saved DB with seed
      const base = (saved && typeof saved === "object" && !Array.isArray(saved)) ? saved : {};
      const merged = {...EXERCISE_DB_SEED};
      // Keep user's existing history entries, add seed only if not present
      Object.keys(base).forEach(name => {
        if(merged[name]) {
          // Merge history: keep unique by date+type
          ...
          merged[name] = {...merged[name], history:[...merged[name].history, ...userOnly].sort((a,b)=>a.date>b.date?1:-1)};
        } else {
          // User's custom exercise not in seed
  ...
  const sharedProps = {history,exerciseDB,setExerciseDB,dayLogs,setDayLogs,saveDay,todayStr,todayLog,settings,customExercises,setCustomExercises,hiddenExercises,setHiddenExercises,weeklyGoals,setWeeklyGoals,workoutDates,workoutTemplates,setWorkoutTemplates};
...
function ScreenTraining({history,exerciseDB,setExerciseDB,saveDay,todayStr,settings,customExercises,setCustomExercises,hiddenExercises,setHiddenExercises,workoutTemplates,setWorkoutTemplates}) {
  ...
  const saveEditEx = () => {
    ...
    if(newName !== oldName) {
      // Rename: move record in exerciseDB to new key, keep history
      setExerciseDB(prev=>{
        const updated = {...prev};
        if(updated[oldName]) {
          updated[newName] = {...updated[oldName], muscle:editEx.muscle};
    ...
    } else if(editEx.muscle !== (exerciseDB[oldName]?.muscle)) {
      // Only muscle changed
      setExerciseDB(prev=>({...prev,[oldName]:{...prev[oldName],muscle:editEx.muscle}}));
    }
    // Update customExercises for UI list
    if(editEx.isCustom) {
      setCustomExercises(p=>p.map(e=>e.name===oldName?{...e,name:newName,muscle:editEx.muscle}:e));
    }
    setEditEx(null);
  };
      ...
      {phase==="templates" && (
        <div>
          <div style={{margin:"0 16px 8px",fontSize:12,color:"var(--muted)"}}>
            Zaznacz które ćwiczenia mają być domyślnie w danym treningu. Odznaczone będą pominięte.
          </div>
          {["push","pull","fbw"].map(trainingType=>{
            const template = workoutTemplates[trainingType]; // null=all selected
            const baseExercises = EXERCISES[trainingType]||[];
            const customForType = customExercises.filter(e=>!e.isOverride&&baseExercises.every(b=>b.name!==e.name));
            const allForType = [...baseExercises,...customForType].filter(e=>!hiddenExercises.includes(e.name));
            const selectedNames = template || allForType.map(e=>e.name); // null means all
            ...
            return (
              <div key={trainingType} className="card" style={{marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  ...
                  <button
                    onClick={()=>setWorkoutTemplates(p=>({...p,[trainingType]:allSelected?[]:allForType.map(e=>e.name)}))}
                ...
                <button
                  onClick={()=>setWorkoutTemplates(p=>({...p,[trainingType]:null}))}
...
function SessionView({type,history,exerciseDB={},setExerciseDB,todayStr,onBack,saveDay,settings,customExercises=[],hiddenExercises=[],workoutTemplates={}}) {
  ...
  const exList = getExercisesForSession(type, customExercises, hiddenExercises, workoutTemplates);
  ...
  const handleFinish = () => {
    const entries = exList
      .filter(ex => {
        ...
        return weights[key] && !excluded.has(key);
      })
      .map(ex => {
        const key = ex.id||ex.name;
    ...
    if(entries.length) {
      setExerciseDB(prev=>{
        const next = {...prev};
        entries.forEach(e=>{
          if(!next[e.exercise]) next[e.exercise] = {muscle: getMuscle(e.exercise,customExercises,prev), trainings:[type], history:[]};
          // Remove existing entry for same date+type, add new one
          next[e.exercise] = {
            ...next[e.exercise],
            history: [
...
function ScreenCalendar({history,exerciseDB={},dayLogs,workoutDates,weeklyGoals,setWeeklyGoals,customExercises}) {
  ...
  const muscleSets={};
  history.filter(e=>wkDays.includes(e.date)).forEach(e=>{ const mu=getMuscle(e.exercise,customExercises,exerciseDB); if(mu) muscleSets[mu]=(muscleSets[mu]||0)+(Number(e.sets)||3); });
...
function ScreenStats({history,exerciseDB={},dayLogs,customExercises=[]}) {
  ...
  const getExForGroup = (group) => {
    if(["push","pull","fbw"].includes(group)){
      ...
      history.forEach(e=>{ const mu=getMuscle(e.exercise,customExercises,exerciseDB); if(mu&&musInGroup.includes(mu)&&!base.includes(e.exercise)&&!extra.includes(e.exercise)) histExtra.add(e.exercise); });
      return [...base,...extra,...histExtra];
    }
    const base=[...new Set(allEx.filter(e=>e.muscle===group).map(e=>e.name))];
    const histExtra=new Set();
    history.forEach(e=>{ const mu=getMuscle(e.exercise,customExercises,exerciseDB); if(mu===group&&!base.includes(e.exercise)) histExtra.add(e.exercise); });
  ...
  const getMuscleFn = (name) => getMuscle(name, customExercises, exerciseDB);
  const getLastMuscle=(muscle)=>{
    const names=new Set(allEx.filter(e=>e.muscle===muscle).map(e=>e.name));
    history.forEach(e=>{ const mu=getMuscle(e.exercise,customExercises,exerciseDB); if(mu===muscle) names.add(e.exercise); });
